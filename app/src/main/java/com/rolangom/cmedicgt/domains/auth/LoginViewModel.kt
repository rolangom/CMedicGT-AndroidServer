...
sealed class LoginEvent(val severity: EventSeverity, val message: String) {
    class GoToHome(severity: EventSeverity, message: String) : LoginEvent(severity, message)
...
class LoginViewModel : ViewModel() {
    ...
    private val authRepository: AuthRepo = RealmAuthRepo
    ...
    fun login(email: String, password: String, fromCreation: Boolean = false) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            runCatching {
                ...
                _event.emit(LoginEvent.GoToHome(EventSeverity.INFO, "User logged in successfully."))
            }.onFailure { ex: Throwable ->
                _state.value = state.value.copy(enabled = true)
                val message = when (ex) {
